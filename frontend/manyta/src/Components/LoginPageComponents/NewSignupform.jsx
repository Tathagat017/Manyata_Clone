...
import { Box, Button, HStack, Image, Input, Stack, VStack , Alert,
    AlertIcon,
    AlertTitle,
...
<VStack rowGap={'1.5rem'}>
    ...
    {setloading1(true);dispatch(signup(signdetails))
        .then((res)=>{
            //console.log(res,"res");
       if(res.type==="SIGN_SUCCESS"){
        if(res.payload==="new user added successfully"){
            errsucc.current=true;
            settoggle(!toggle);
            setchecksignuppage(false)
        }
        else if(res.payload==="few fields are missing"){
          errsucc.current=false;  
          settoggle(!toggle);
        }
       }
       else if(res.type==="SIGN_FAILURE"){
        errsucc.current=false;
        settoggle(!toggle);
    }
    setloading1(false)
        ...
        {errsucc.current==false?<Alert status='error'>
    <AlertIcon />
    There was an error processing your request
  </Alert>:null}

  {errsucc.current?<Alert status='success'>
    <AlertIcon />
